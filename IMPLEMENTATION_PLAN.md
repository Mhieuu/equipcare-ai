...
> **Trạng thái M2 (12/09/2026)**: ✅ HOÀN THÀNH API layer.
> - Migration `0002_asset`: CHECK constraint `manual_state IN ('NORMAL','SUSPENDED','RETIRED')` + view `v_asset_state` (skeleton M2; M5 JOIN work_orders để thêm MAINTENANCE/REPAIR).
> - Backend module `asset`: CRUD + lifecycle state machine + QR PNG base64 + activity_status mapping tiếng Việt.
> - `backend-core/asset.machine.ts`: `canTransitionManualState` / `assertManualStateTransition` / `assertAssetCanHaveWorkOrder` (helper cho M5) / `deriveActivityStatus`.
> - `packages/shared/asset.enum.ts`: `AssetManualState` + `ActivityStatus` enums. `labels.ts`: `ActivityStatusLabel` + `AssetManualStateLabel` tiếng Việt.
> - Permission: `asset:read` / `asset:create` / `asset:update` (đã có trong seed).
> - Audit: `asset.create` / `asset.update` / `asset.state.change` (state_reason lưu lý do RETIRED).
> - E2E: **6 suites / 29 tests PASS** (5 TC-ASSET mới + 24 M1). Test verify RETIRED terminal + RETIRED thiếu reason → 422 + QR PNG data URL.
> - Lint 0 warning, typecheck 5/5, build PASS.
> - **Hoãn M2**: UI tabs (Tổng quan/Sự cố/Công việc/Tài liệu) — sẽ làm ở milestone UI.
...
> **Trạng thái M3 (12/09/2026)**: ✅ HOÀN THÀNH API layer.
> - Migration `0003_attachments`: 3 CHECK constraint — `files.storage_state ∈ (STAGED/READY/REJECTED)`, `attachment_links` đúng 1 parent, `technical_documents.document_type ∈ (MANUAL/SCHEMATIC/PROCEDURE/OTHER)`.
> - Backend-core: `file-validator.ts` (magic bytes per MIME, size 25MB, sha256), `storage-adapter.ts` (LocalFsAdapter + assertSafeKey chống path traversal, gồm chuẩn hóa `'\\'`↔`'/'` cho Windows).
> - Module `attachment`: `POST /files` (multipart → STAGED), `GET /files/:id`, `POST /files/:id/link` (Tx STAGED→READY), `GET /files/:id/download`, `DELETE /files/:id` (soft unlink), `cleanupStaged()` cho worker cron.
> - Module `technical-documents`: CRUD metadata + version (`document_versions` auto-increment) + per-doc RBAC (`document_roles`, idempotent grant).
> - Permission: `attachment:upload`, `attachment:read`. Document dùng chung `asset:read`/`asset:update` (P1 nên tách).
> - Audit: `attachment.upload`, `attachment.upload.rejected`, `attachment.link`, `attachment.unlink`, `technical-document.create/update`, `version.add`, `role.grant/revoke`.
> - E2E: **7 suites / 34 tests PASS** (5 TC-DOC mới + 29 cũ).
> - Lint 0 warning, typecheck 5/5, build PASS.
> - **Hoãn M3**: cron worker chạy `cleanupStaged()` (M3 có helper; cần wire vào worker interval ở M9/M10). MinIO adapter (M3 dùng LocalFs; interface `StorageAdapter` chừa sẵn để swap). UI attachment modal (làm ở milestone UI).
...
> **Trạng thái M4 (13/09/2026)**: ✅ HOÀN THÀNH API layer.
> - Migration `0004_incidents_ai`: 5 CHECK constraint — `incidents.status`, `incidents.priority_code`, `incident_messages.message_type`, `ai_requests.task_type`, `ai_requests.status` (Doc04 §3.2 + §5.8 enum strict).
> - Backend-core: `state-machine/incident.machine.ts` (NEW↔AWAITING_INFO↔IN_PROGRESS→RESOLVED→CLOSED | CANCELLED; CLOSED/CANCELLED terminal), `ai/ai-provider.ts` (interface + AbortError), `ai/mock-ai-provider.ts` (heuristic INCIDENT_TRIAGE/ASSET_SUMMARY, 100-300ms latency, AbortSignal-aware), `ai/provider.factory.ts` (AI_PROVIDER=mock default).
> - Module `incident`: `POST /incidents` (tạo + REPORTER message, code `INC-yyyymmdd-XXXXX`), `GET /incidents` (filter status/priority/assetId, paginated), `GET /incidents/:id`, `POST /incidents/:id/transition` (state machine + side-effects RESOLVED_CLOSED/CANCELLED + SYSTEM message), `POST /incidents/:id/messages` (STAFF auto AWAITING_INFO→IN_PROGRESS, refuse terminal).
> - Module `ai`: `POST /ai/incidents/:id/analyze` (HTTP **202** + `{requestId, status:QUEUED}`), `GET /ai/requests/:id` (poll status/output, owner check 403). Mock provider `MockAiProvider.analyze()` chạy in-process (worker pattern đơn giản — BullMQ integration ở M5/M6 nếu có Redis); retry exponential backoff (1s/2s/4s cap 8s) tối đa `ai.max_retries`; timeout `ai.timeout_ms` → `TIMED_OUT`. Tunables từ `system_settings`.
> - Permission: thêm `incident:transition`, `incident:message:create` (ADMIN/MANAGER/TECHNICIAN seed).
> - Audit: `incident.create`, `incident.transition` (old/newValue status + reason), `ai.request.create` (correlationKey=requestId).
> - Input snapshot lọc PII (Doc05 §10.1): chỉ assetCode/assetName/assetTypeCode + description/impactDescription cắt 500 ký tự; KHÔNG log API key.
> - E2E: **9 suites / 47 tests PASS** (7 TC-INC mới + 6 TC-AI + 34 cũ).
> - Lint 0 warning, typecheck 5/5, build PASS.
> - **Hoãn M4**: BullMQ/Redis worker (interface đã chừa; chạy in-process); UI incident queue + AI gợi ý panel (làm ở milestone UI); OpenAI provider impl (M5+ hardening); hot-reload `ai.*` settings.
...
> **Trạng thái M5 (13/09/2026)**: ✅ HOÀN THÀNH API layer.
> - Migration `0005_work_orders`: 5 CHECK constraint — `work_orders.status` (NEW/ASSIGNED/IN_PROGRESS/COMPLETED/CANCELLED — Doc04 §3.2, **không PAUSED**), `work_orders.kind` (REPAIR/MAINTENANCE/INSPECTION), `work_orders.creation_mode` (MANUAL/FROM_INCIDENT/FROM_MAINTENANCE), `work_orders.priority_code` (LOW/MEDIUM/HIGH/CRITICAL), `work_order_notes.note_type` (PROGRESS/PAUSE_START/PAUSE_END/WAITING_APPROVAL_START/END). Partial unique index `uniq_open_repair_per_incident` (Doc04 DB-03 — REPAIR + chưa terminal). 3 index bổ sung (`status+due_at`, `active started`, `notes timeline`).
> - Backend-core: `state-machine/work-order.machine.ts` (NEW→ASSIGNED→IN_PROGRESS→COMPLETED|CANCELLED; terminal; cancel require reason; markStarted/Completed/Cancelled). `sla/sla.engine.ts` pause-aware: `computeActiveElapsed(slaStartedAt, pauseIntervals, waitingApprovalIntervals)`; `isOverdue(status, slaSeconds, activeElapsed)`; **WAITING_APPROVAL đã bỏ khỏi OPEN_STATUSES** (M6 mới dùng).
> - Module `work-order`: `POST /work-orders` (auto code `WO-yyyymmdd-XXXXX`, department_id_snapshot từ asset, due_at mặc định = now + SLA(priority), P2002 partial unique → 409), `PATCH /:id/assign`, `PATCH /:id/transition` (state machine + optimistic lock row_version + side-effects timestamps), `POST /:id/complete` (chỉ từ IN_PROGRESS, save result_summary/action_taken/confirmedCause/checklistResults), `POST /:id/cancel` (require reason), `POST /:id/notes` (PROGRESS hoặc PAUSE_START yêu cầu pauseReason, PAUSE_END check có PAUSE_START mở → 422 WO_RESUME_WITHOUT_PAUSE), `GET /:id/sla-status` (activeElapsed, remainingSeconds, pauseSeconds, isOverdue).
> - Auto-resolve incident: WO REPAIR `COMPLETED` → check `count(work_orders where incident_id=X AND kind='REPAIR' AND status IN (NEW,ASSIGNED,IN_PROGRESS))=0` → incident tu chuyển `RESOLVED` + `incident_messages` (SYSTEM) + audit `incident.auto_resolve`. Transaction 2 bảng atomic.
> - Shared: thêm `WorkOrderCreationMode`, `WorkOrderNoteType` + labels VI. Bỏ `WAITING_APPROVAL` khỏi `WorkOrderStatus` enum (enum status Doc04 strict đồng bộ CHECK constraint).
> - Seed fix: thêm `asset_types.bootstrap` (Doc04 — bị thiếu từ M1 gây fail asset CRUD E2E).
> - Audit: `work_order.create`, `work_order.transition` (old/newValue status + reason), `work_order.assign`, `work_order.complete`, `work_order.note` (noteType + pauseReason), `incident.auto_resolve` (correlationKey=incidentId).
> - E2E M5: **8 tests PASS** (TC-WO-01..04 core, TC-WO-CONCURRENCY optimistic lock, TC-WO-AUTO-RESOLVE, TC-WO-PARTIAL-UNIQUE, TC-WO-STUB list filter). **Total: 10 suites / 55 tests PASS**.
> - Lint 0 warning, typecheck 5/5, build 5/5.
> - **Hoãn M5**: WAITING_APPROVAL + Approval chain (M6); UI Kanban + pause modal (M5 UI riêng); Maintenance WO linking (M8); checklist UI runtime (UI phase); hot-reload SLA settings (M9).
...
> **Trạng thái M7 (13/09/2026)**: ✅ HOÀN THÀNH API layer.
> - Migration `0007_inventory`: CHECK `stock_transactions.movement_type` (ISSUE/RETURN/RECEIPT/ADJUSTMENT/TRANSFER_IN/TRANSFER_OUT — Doc04 §5.6 strict); `parts.on_hand >= 0` + `parts.minimum_stock >= 0` (defense-in-depth). Index `idx_parts_low_stock` (partial WHERE is_active=true), `idx_work_order_parts_wo`. **Trigger `notify_low_stock` (AFTER INSERT/UPDATE OF on_hand)** → `notifications(event_type='INVENTORY_LOW_STOCK', event_key='low_stock:{part_id}')` cho admin recipients (Doc04 §5.8 UNIQUE(recipient_id, event_key) idempotent).
> - Backend-core `stock/stock.domain.ts`: `computeNetIssued(entries)` (Q-06 SUM(ISSUE) − SUM(RETURN) per approval_revision); `checkNetIssuedAgainstBudget` (raise APR_EXCEEDS_REVISION_BUDGET_QTY); `movementSign` (ADJUSTMENT co the am); `assertReturnHasOriginal`; `sumReturnsForIssue`. Inbound/Outbound classifier.
> - Shared: `StockTransactionType` enum + labels VI. Permission `INVENTORY_TRANSFER`, `INVENTORY_ADJUST`, `INVENTORY_RETURN` (ADMIN/MANAGER/TECHNICIAN).
> - Module `part` (apps/api): `POST /parts` (code unique 409, FK dept/location check), `GET /parts?isActive&lowStock&departmentId`, `GET /parts/:id`, `PATCH /parts/:id` (optimistic lock row_version, audit part.update).
> - Module `stock-transaction` (apps/api):
>   - `POST /work-orders/:woId/parts/issue` (atomic `UPDATE parts SET on_hand = on_hand + delta RETURNING on_hand`; check `parts_on_hand_non_negative` → STOCK_INSUFFICIENT 422; neu `part.requires_approval` thi lookup latest APPROVED revision → `assertWithinIssuedBudget` → APR_EXCEEDS_REVISION_BUDGET_QTY 422; co set `unit_price_snapshot` tu part.reference_price).
>   - `POST /work-orders/:woId/parts/return` (lookup original ISSUE, `original_work_order_id` cross-check WO, sum prev returns, check RETURN <= remaining → STOCK_RETURN_EXCEEDS_ISSUE 422; neu original sai movement_type → STOCK_RETURN_INVALID_ORIGINAL 422; cap on_hand tang tuong ung).
>   - `POST /stock-transactions/receipt` (RECEIPT tang on_hand, cap decimal>=0).
>   - `POST /stock-transactions/adjust` (ADJUSTMENT +/-; reason >=3 ky tu → STOCK_ADJUSTMENT_REQUIRES_REASON 422; signed quantity).
>   - `POST /parts/:id/adjust` (alias cho SCR-PART-05 - cùng service, partId resolve từ path).
>   - `GET /stock-transactions?partId&workOrderId` (ledger view, orderBy occurred_at DESC).
>   - Atomic transaction (Serializable isolation) + audit (stock.issue/return/receipt/adjust).
> - Module `work-order` (apps/api, M7 nested): `GET /work-orders/:id/parts/planned` (list), `POST /work-orders/:id/parts/planned` (upsert UNIQUE(wo,part), audit work_order.plan_part).
> - Seed: them 5 permissions moi (INVENTORY_TRANSFER/ADJUST/RETURN).
...
> - Backend-core:
>   - `state-machine/approval.machine.ts` (DRAFT↔SUBMITTED↔APPROVED|REJECTED|INFO_REQUESTED|CANCELLED; APPROVED/REJECTED/INFO_REQUESTED → DRAFT cho new revision; resolveTargetState(current, action)).
>   - `state-machine/work-order.machine.ts` thêm WAITING_APPROVAL (IN_PROGRESS↔WAITING_APPROVAL).
>   - `cost/cost.domain.ts` (Q-06 net_cost aggregation; `checkNetCostAgainstRevision`; `summarizeNetCost` partition PART/LABOR/OTHER/credit).
> - Shared: thêm `ApprovalStatus` (Doc04), `ApprovalEventType`, `CostCategory`, `CostDirection` + labels VI. WorkOrderStatus thêm WAITING_APPROVAL. Thêm permission `APPROVAL_UPDATE_DRAFT`, `APPROVAL_SUBMIT`, `APPROVAL_REQUEST_INFO`, `APPROVAL_CANCEL`, `COST_CREATE`.
> - Module `cost-entry` (apps/api): `POST /work-orders/:id/cost-entries` (PART/LABOR/OTHER, DEBIT/CREDIT + Q-06 check khi `approval_revision_id` thuộc SUBMITTED/APPROVED), `GET /work-orders/:id/cost-entries` (list + summary {partDebit, laborDebit, otherDebit, credit, netCost}).
> - Module `approval` (apps/api):
>   - `POST /approvals` (tạo DRAFT + revision_no=1; conflict 1 WO = 1 approval active; reject terminal WO).
>   - `PATCH /approvals/:id/draft` (update reason/action_plan/budget; chỉ proposer).
>   - `POST /approvals/:id/revisions` (mở revision mới khi APPROVED/REJECTED/INFO_REQUESTED; q.c. proposer only).
>   - `PATCH /approvals/:id` (universal action {SUBMITTED|APPROVED|REJECTED|INFO_REQUESTED|CANCELLED} → state machine; optimistic lock row_version R-04; self-approval guard 422 APR_SELF_APPROVAL_FORBIDDEN; WO coupling WAITING_APPROVAL).
>   - `GET /approvals`, `GET /approvals/:id` (inbox + history events + revisions + parts).
>   - WO coupling transaction: SUBMITTED → WAITING_APPROVAL_START note + WO.status WAITING_APPROVAL. APPROVED/REJECTED/CANCELLED → WAITING_APPROVAL_END note + WO.status IN_PROGRESS. INFO_REQUESTED → ghi note PROGRESS (WO vẫn ở WAITING_APPROVAL).
> - Seed: thêm `APPROVAL_UPDATE_DRAFT`, `APPROVAL_SUBMIT`, `APPROVAL_REQUEST_INFO`, `APPROVAL_CANCEL`, `COST_CREATE` vào matrix; ADMIN + MANAGER + TECHNICIAN.
> - Audit: `approval.draft_created`, `approval.draft_updated`, `approval.{action}`, `approval.new_revision`, `cost_entry.create`.
> - E2E M6: **10 tests PASS** (TC-COST-01..03 + TC-APR-01..07). **Total: 11 suites / 65 tests PASS**.
> - Lint 0 warning, typecheck 5/5, build 5/5.
> - **Hoãn M6**: UI approval inbox (SCR-APR-01a) + cost form (M6 UI); CSV/PDF export approval history; revision_part_diff UI; tích hợp `net_issued_quantity` với parts (M7); Revoked action (admin tooling — chưa cần ở M6).
...
| Hạng mục | Kết quả |
|---|---|
| Migration Prisma | **10/10 đã apply** (`0001_init` → `0009_notif_dashboard`) |
| PostgreSQL CHECK constraints | **30** CHECK (Doc04 strict enum + business invariants; +2 notifications enum constraints M9) |
| PostgreSQL triggers | 3 (FR-APR-09 self-approval, low-stock UPDATE, low-stock INSERT) |
| Modules NestJS | **19** (M9: +notification, +dashboard, +report, +realtime) |
| Controllers | 19 với 100+ endpoints |
| Permissions (seed) | **49** (Admin + Manager + Technician matrix) |
| Audit actions | 30+ action types (+ notification.read, notification.read_all) |
| E2E test files | 14 files (M9: +notif-dashboard-report.e2e-spec.ts) |
| E2E tests | **100/100 PASS** |
| Lint | 0 warning |
| Build | 5/5 packages PASS |

### 13.2. Timeline thực tế vs kế hoạch

| Milestone | Plan (Tuần) | Thực tế | Ngày chốt | Trạng thái |
|---|---|---|---|---|
| M0 Baseline | Trước W1 | ✅ | 11/09/2026 | HOÀN THÀNH |
| M1 Foundation + Auth + IAM + Org | W1–W2 | ✅ | 12/09/2026 | HOÀN THÀNH |
| M2 Asset + QR + lifecycle | W3–W4 | ✅ | 12/09/2026 | HOÀN THÀNH |
| M3 Attachment + Technical Docs | W5 | ✅ | 12/09/2026 | HOÀN THÀNH |
| M4 Incident + AI async | W6–W7 | ✅ | 12/09/2026 | HOÀN THÀNH |
...
| # | Vấn đề | Xử lý |
|---|---|---|
| 1 | Endpoint `POST /parts/:id/adjust` thiếu (plan M7 liệt kê) | Đã thêm — alias cho `POST /stock-transactions/adjust`, partId resolve từ path. |
| 2 | `PATCH /approvals/:id` (universal action) thiếu `@Permissions` | Đã thêm `@Permissions(Permission.APPROVAL_DECIDE)` — admin/manager OK, technician sẽ 403. |

### 13.5. Items chưa xử lý / để M8+
